...
//
// Current routing policy (see openai_compat.ShouldUseResponsesAPI):
//   - Newly created API-key accounts receive an explicit Chat Completions default.
//   - CC→Responses conversion is enabled only by trusted account configuration;
//     probe metadata never upgrades routing automatically.
//   - Legacy accounts without a setting preserve their historical behavior.
//   - OAuth accounts keep their native Responses path.
func (s *OpenAIGatewayService) ForwardAsChatCompletions(
	...
	// New API-key accounts are persisted with an explicit false capability and
	// take this raw Chat path. Legacy unset accounts retain historical routing.
